...
\usepackage{fancyhdr, amsmath, graphicx, hyperref, multirow}
...
\begin{enumerate}[label=J\arabic*., leftmargin=*]
	\item Z. Hoobakht\footnotemark[3], H. Gangammanavar, and D.\ Rajan, Optimal Spectral Allocation in Citizens Broadband Radio Service, \ifpublic accepted for publication in \textit{IEEE Transactions on Cognitive Communications and Networking}, 2024.
...
\begin{enumerate}[label=R\arabic*., leftmargin=*] 
	...
	\item N.\ Sakhavand\footnotemark[3], J.\ Rosenberger, V.\ Chen, and H.\ Gangammanavar, Design of Experiments for the Stochastic Unit Commitment with Economic Dispatch Models, \ifpublic 2022 (third review). \else under third review for publication in \textit{European Journal on Computational Optimization} (first submission: 06/2022). \fi 
...
%	\item S.\ Tabrizian\footnotemark[2], H.\ Gangammanavar, and H.\ \"{U}ster, A Sampling-based Method for Stochastic Mixed-Integer Programming with Application in Portfolio Optimization, 2022 (target  \textit{Computational Management Science}).
%	\item M.\ Bansal and H.\ Gangammanavar, Randomized Dual Decomposition for Stochastic Mixed-integer Programming, 2022 (target \textit{Operations Research Letters}).
%	\item H.\ Gangammanavar and S.\ Sen, Data-driven policies for multistage stochastic programs and applications in power systems (target \emph{Operations Research}, invited).
	% \item H.\ Gangammanavar, Regularized Internal Sampling Algorithm for Two-stage Stochastic Linear Programs (target \textit{Mathematical Programming}). 
	%	\item H.\ Gangammanavar, A Sampling-based Algorithm for Two-Stage Stochastic Nash Games (target \textit{Mathematical Programming}).	
\end{enumerate}
